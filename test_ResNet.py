...
def load_model_from_checkpoint(model_type="resnet18", checkpoint_path=None, num_classes=None, device=None,
                               results_path=None):
    """从检查点加载ResNet模型"""
    if results_path is None:
        results_path = f"./results/AppClassNet/top200/ResNet/1"  # 默认结果路径
        ...
        for inputs, labels in data_loader:
            ...
            if collect_features:
                ...
                features = model.avgpool(model.layer4(model.layer3(model.layer2(model.layer1(
                    model.maxpool(model.relu(model.bn1(model.conv1(inputs)))))))))
