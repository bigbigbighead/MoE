...
class MoE4Router(nn.Module):
    ...
    def __init__(self, feat_dim, num_experts=3, routing_type='softmax'):
        super().__init__()
        self.routing_type = routing_type
...
class SpecializedExpert(nn.Module):
    ...
    def __init__(self, feat_dim, class_range):
        """
        Args:
            feat_dim: 特征维度
...
class MoE4Model(nn.Module):
    ...
    def __init__(self, total_classes=200, class_ranges=None, routing_type='hard',
                 input_channels=1, input_height=1, input_width=1024):
        super().__init__()
        ...
        # 路由器
        self.router = MoE4Router(feat_dim, self.num_experts, routing_type)

        # 创建专家
        self.experts = nn.ModuleList()
        for class_range in self.class_ranges:
            expert = SpecializedExpert(feat_dim, class_range)
