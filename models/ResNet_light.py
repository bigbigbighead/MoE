...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, input_channels=1, input_height=1, input_width=1024):
        ...
        if input_height == 1:
            # 使用1x3的卷积核，不改变高度维度
            self.conv1 = nn.Conv2d(input_channels, 8, kernel_size=(1, 7), stride=(1, 2), padding=(0, 3), bias=False)
        else:
            # 原始7x7卷积
            self.conv1 = nn.Conv2d(input_channels, 8, kernel_size=7, stride=2, padding=3, bias=False)
